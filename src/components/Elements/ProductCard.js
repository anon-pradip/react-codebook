...
const ProductCard = ({ product }) => {
  const { id, name, overview, long_description, price, poster, image_local, rating, best_seller } = product
  ...
  return (
    <div className="m-3 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
      <Link to={`products/${id}`} className="relative" >
        ...
        <img className="rounded-t-lg w-full h-64" src={poster} alt='test-for-now' />
      </Link>
      <div className="p-5">
        <Link to={`products/${id}`}>
